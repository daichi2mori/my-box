...
func Cover(c *fiber.Ctx) error {
	filePath, err := utils.BuildFilePath(c.Params("path"), utils.GetENV("MANGA_DIR"), utils.GetENV("MANGA_SEARCH"))
	if err != nil {
...
func Page(c *fiber.Ctx) error {
	...
	files, err := utils.CollectFiles(utils.GetENV("MANGA_DIR"), utils.GetENV("MANGA_SEARCH"))
	if err != nil {
	...
	for _, str := range files {
		if strings.HasSuffix(str, "01") {
			result = append(result, str)
		}
	}

	chunks := utils.MakeChunks(result)
...
func Author(c *fiber.Ctx) error {
	normalizedPath := utils.BuildNormalizedPath(c.Params("path"), utils.GetENV("MANGA_DIR"))
	files, err := utils.CollectFiles(normalizedPath, utils.GetENV("MANGA_SEARCH"))
	if err != nil {
		return c.Status(500).SendString("エラー：" + err.Error())
	}
